...
//var contractAddress = '0x2CA950C21e5fe72278834730F11ef730c1C20441'
...
function donate() {
  if ( isNaN($("#donation_input").val())
   || $("#donation_input").val() < 1 || $("#donation_input").val() > 20000  ){
    return false;
  }

  web3.eth.sendTransaction({from: donor, to: contractAddress, value: Math.floor($("#donation_input").val())}).then(function(result) {
...
function provideStake(forestId) {
...
var receipt =  myContract.methods.provideStake(forestId).send({from: staker, value: stake, gas: 500000000});
console.log(receipt);
